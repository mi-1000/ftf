...
def is_line_starting_with_liber(text: str) -> bool:
    """Returns True if the line starts with 'Liber'."""
    return text.startswith("Liber")

def starts_with_letter_or_number(text: str) -> bool:
    """Returns True if the line starts with a letter or number."""
...
async def fetch_and_save_text(url: str, author_name: str):
    ...
    if response.status_code == 200:  # Status ok
        ...
        for cell in cells:
            ...
            # Ignore cells that contain numbers, punctuation, start with 'Non.', or start with 'Liber'
            if (contains_numbers(cell_text) or 
                is_only_punctuation_or_numbers(cell_text) or 
                is_line_starting_with_non(cell_text) or 
                is_line_starting_with_liber(cell_text)):
                continue
...
def save_text_to_file(title: str, content: List[str], author_name: str):
    ...
    # Remove lines that contain numbers, punctuation, start with 'Non.', start with 'Liber', or are empty
    text_lines = text_content.splitlines()
    filtered_lines = [
        ...
        if not contains_numbers(line) and 
           ...
           not is_line_starting_with_liber(line) and
           line and starts_with_letter_or_number(line)
    ]
    text_content = "\n".join(filtered_lines).strip()

    # Write to the file if there's any content left
    if text_content:
